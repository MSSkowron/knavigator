...
unified_job_created_timestamp_seconds = Gauge(
    'unified_job_created_timestamp_seconds', 'Timestamp when the Job (K8s or Volcano) was created', LABELS, registry=REGISTRY
)
unified_job_start_timestamp_seconds = Gauge(
    'unified_job_start_timestamp_seconds', 'Timestamp when the Job first started running', LABELS, registry=REGISTRY
)
unified_job_completion_timestamp_seconds = Gauge(
    'unified_job_completion_timestamp_seconds', 'Timestamp when the Job completed or failed', COMPLETION_LABELS, registry=REGISTRY
)
unified_job_wait_duration_seconds = Gauge(
    'unified_job_wait_duration_seconds', 'Time duration between creation and start of the job in seconds', LABELS, registry=REGISTRY
)
unified_job_execution_duration_seconds = Gauge(
    'unified_job_execution_duration_seconds', 'Time duration between start and completion/failure of the job in seconds', COMPLETION_LABELS, registry=REGISTRY
)
unified_job_total_duration_seconds = Gauge(
    'unified_job_total_duration_seconds', 'Time duration between creation and completion/failure of the job in seconds', COMPLETION_LABELS, registry=REGISTRY
)
JOB_STATUS_MAP = {
    'Pending': 0, 'Running': 1, 'Succeeded': 2, 'Failed': 3,
    'Completed': 2, 'Terminating': 4, 'Terminated': 5,
    'Aborting': 6, 'Aborted': 7, 'Unknown': -1
}
unified_job_status_phase = Gauge(
    'unified_job_status_phase', 'Current phase of the Job (Pending=0, Running=1, Succeeded=2, Failed=3, ...)', LABELS, registry=REGISTRY
)
...
def parse_timestamp(ts_input):
    ...
    if isinstance(ts_input, str):
        try:
            ...
            logging.warning(f"Could not parse timestamp string: {ts_input}", exc_info=False)
            return None
...
def get_k8s_job_final_status(conditions):
    ...
    valid_conditions = [c for c in conditions if c and hasattr(c, 'last_transition_time') and hasattr(c, 'type') and hasattr(c, 'status')]
    # Sortuj po czasie malejąco
    sorted_conditions = sorted(
        valid_conditions,
        key=lambda x: x.last_transition_time or datetime(1970, 1, 1, tzinfo=timezone.utc),
        reverse=True
    )
...
def get_volcano_start_time(uid, conditions_list):
    ...
    if start_time_ts:
        logging.debug(f"[get_volcano_start_time] UID: {uid} - Found start time in cache: {start_time_ts}")
    ...
    if conditions_list and isinstance(conditions_list, list):
        for condition in conditions_list:
             if isinstance(condition, dict) and condition.get('status') == 'Running':
                ...
                if ts:
                    logging.debug(f"[get_volcano_start_time] UID: {uid} - Found 'Running' condition with time: {ts}")
                    if earliest_running_time is None or ts < earliest_running_time:
                        earliest_running_time = ts
                else:
                     logging.debug(f"[get_volcano_start_time] UID: {uid} - Found 'Running' condition but failed to parse time: {condition.get('lastTransitionTime')}")


    if earliest_running_time:
        logging.debug(f"[get_volcano_start_time] UID: {uid} - Determined earliest start time: {earliest_running_time}, caching.")
        volcano_start_times_cache[uid] = earliest_running_time.timestamp()
        return earliest_running_time
    else:
         logging.debug(f"[get_volcano_start_time] UID: {uid} - No 'Running' condition found.")
         return None
...
def process_job(job_obj):
    ...
    try:
        ...
        if isinstance(job_obj, kubernetes.client.V1Job):
        ...
        elif isinstance(job_obj, dict) and job_obj.get('apiVersion', '').startswith("batch.volcano.sh/"):
            job_kind = "VolcanoJob"
            ...
            logging.warning(f"{log_prefix} Skipping unknown job object type or missing apiVersion: {type(job_obj)}")
            return None
        ...
        base_labels = {"job_name": name, "namespace": namespace, "uid": uid, "job_kind": job_kind}
        ...
        if creation_time_dt:
            logging.debug(f"{log_prefix} Parsed creation_time_dt: {creation_time_dt.isoformat()}")
        else:
            logging.warning(f"{log_prefix} Failed to parse or missing creation timestamp.")
        ...
        if job_kind == "Job":
            ...
            logging.debug(f"{log_prefix} Raw completion_time_input: {completion_time_input}")

            start_time_dt = parse_timestamp(start_time_input)
            completion_time_dt = parse_timestamp(completion_time_input)
            final_status_cond, completion_time_cond = get_k8s_job_final_status(conditions)

            if completion_time_dt is None and completion_time_cond:
                 logging.debug(f"{log_prefix} Using completion time from conditions: {completion_time_cond}")
                 completion_time_dt = completion_time_cond
            if final_status_cond:
                final_status = final_status_cond
                logging.debug(f"{log_prefix} Determined final status from conditions: {final_status}")
            ...
            logging.debug(f"{log_prefix} K8s status counts: succeeded={succeeded_count}, failed={failed_count}, active={active_count}, suspended={is_suspended}")

            if succeeded_count > 0: current_phase_str = 'Succeeded'
            elif failed_count > 0: current_phase_str = 'Failed'
            elif active_count > 0: current_phase_str = 'Running'
            elif is_suspended: current_phase_str = 'Pending'
            elif creation_time_dt: current_phase_str = 'Pending'
            else: current_phase_str = 'Unknown'
            logging.debug(f"{log_prefix} Determined current_phase_str: {current_phase_str}")
            ...
            start_time_dt = get_volcano_start_time(uid, conditions_list) # Logowanie jest wewnątrz tej funkcji
            current_phase_str = status_data.get('state', {}).get('phase', 'Unknown')
            logging.debug(f"{log_prefix} Volcano current_phase_str: {current_phase_str}")

            if current_phase_str in ['Completed', 'Failed']:
                completion_time_input = status_data.get('state', {}).get('lastTransitionTime')
                logging.debug(f"{log_prefix} Raw completion_time_input (from state): {completion_time_input}")
                completion_time_dt = parse_timestamp(completion_time_input)
                final_status = 'Succeeded' if current_phase_str == 'Completed' else 'Failed'
                if completion_time_dt is None:
                     logging.warning(f"{log_prefix} Could not parse completion time for phase {current_phase_str}")
                else:
                     logging.debug(f"{log_prefix} Determined final status: {final_status} at {completion_time_dt.isoformat()}")

        # --- Koniec części specyficznej dla typu ---

        # --- Przetwarzanie i Eksport Metryk (wspólne) ---
        logging.debug(f"{log_prefix} Final Timestamps - Creation: {creation_time_dt}, Start: {start_time_dt}, Completion: {completion_time_dt}")
        ...
        if start_time_dt:
            ...
            if creation_time_dt:
                 ...
                 if wait_duration < 0:
                      logging.warning(f"{log_prefix} Negative wait duration calculated ({wait_duration}), setting to 0.")
                      wait_duration = 0
                 logging.debug(f"{log_prefix} Exporting wait_duration: {wait_duration}")
                 unified_job_wait_duration_seconds.labels(**base_labels).set(wait_duration)
            else:
                 logging.debug(f"{log_prefix} Cannot calculate wait_duration, missing creation_time_dt.")
        ...
        if completion_time_dt and final_status:
            ...
            logging.debug(f"{log_prefix} Exporting completion_timestamp (status={final_status}): {ts_val}")
            unified_job_completion_timestamp_seconds.labels(**completion_labels).set(ts_val)
            previously_exported_completion_metrics.setdefault(uid, set()).add('completion')

            if start_time_dt:
                execution_duration = (completion_time_dt - start_time_dt).total_seconds()
                if execution_duration < 0:
                    logging.warning(f"{log_prefix} Negative execution duration calculated ({execution_duration}), setting to 0.")
                    execution_duration = 0
                logging.debug(f"{log_prefix} Exporting execution_duration (status={final_status}): {execution_duration}")
                unified_job_execution_duration_seconds.labels(**completion_labels).set(execution_duration)
                previously_exported_completion_metrics.setdefault(uid, set()).add('duration_exec')
            else:
                 logging.debug(f"{log_prefix} Cannot calculate execution_duration, missing start_time_dt.")

            if creation_time_dt:
                total_duration = (completion_time_dt - creation_time_dt).total_seconds()
                if total_duration < 0:
                    logging.warning(f"{log_prefix} Negative total duration calculated ({total_duration}), setting to 0.")
                    total_duration = 0
                logging.debug(f"{log_prefix} Exporting total_duration (status={final_status}): {total_duration}")
                unified_job_total_duration_seconds.labels(**completion_labels).set(total_duration)
                previously_exported_completion_metrics.setdefault(uid, set()).add('duration_total')
            else:
                 logging.debug(f"{log_prefix} Cannot calculate total_duration, missing creation_time_dt.")

        phase_value = JOB_STATUS_MAP.get(current_phase_str, JOB_STATUS_MAP['Unknown'])
        logging.debug(f"{log_prefix} Exporting status_phase ({current_phase_str}): {phase_value}")
        ...
        logging.error(f"{log_prefix} UNEXPECTED error during processing job {job_ns_safe}/{job_name_safe} (UID: {uid_safe}): {e}", exc_info=True)
...
def collect_metrics():
    ...
    try:
        ...
        try:
            ...
            logging.info("Using incluster config.")
        except kubernetes.config.ConfigException:
            logging.info("Incluster config failed, trying kube config.")
            kubernetes.config.load_kube_config()
            logging.info("Using kube config.")

        api_clients['batch_v1'] = kubernetes.client.BatchV1Api()
        api_clients['custom_objects'] = kubernetes.client.CustomObjectsApi()
        api_clients['core_v1'] = kubernetes.client.CoreV1Api()

        api_clients['core_v1'].get_api_resources()
        ...
        try:
            ...
            if TARGET_NAMESPACE:
                k8s_jobs_list = api_clients['batch_v1'].list_namespaced_job(namespace=TARGET_NAMESPACE, watch=False, timeout_seconds=60)
            else:
                k8s_jobs_list = api_clients['batch_v1'].list_job_for_all_namespaces(watch=False, timeout_seconds=60)
            all_jobs_raw.extend(k8s_jobs_list.items) # Dodaj obiekty V1Job
        ...
        try:
            ...
            if TARGET_NAMESPACE:
                volcano_jobs_list = api_clients['custom_objects'].list_namespaced_custom_object(
                    group="batch.volcano.sh", version="v1alpha1", plural="jobs", namespace=TARGET_NAMESPACE, timeout_seconds=60
                )
            else:
                volcano_jobs_list = api_clients['custom_objects'].list_cluster_custom_object(
                    group="batch.volcano.sh", version="v1alpha1", plural="jobs", timeout_seconds=60
                )
            volcano_items = volcano_jobs_list.get('items', [])
            all_jobs_raw.extend(volcano_items) # Dodaj słowniki VolcanoJob
            logging.info(f"Fetched {len(volcano_items)} Volcano Jobs.")
        except kubernetes.client.ApiException as e:
            if e.status == 404:
                 logging.warning("Volcano CRD (batch.volcano.sh/v1alpha1/jobs) not found in cluster.")
            else:
        ...
        logging.error(f"Error initializing Kubernetes client or API: {e}", exc_info=True)
        return
    ...
    if metrics_to_remove_candidates:
         logging.info(f"Stale completion metrics detected for UIDs: {list(metrics_to_remove_candidates.keys())}. Relying on Prometheus staleness.")

    # --- Czyszczenie Cache Czasu Startu Volcano ---
    volcano_uids_to_remove_from_cache = set(volcano_start_times_cache.keys()) - current_job_uids_processed
    if volcano_uids_to_remove_from_cache:
        logging.info(f"Removing UIDs from Volcano start time cache: {volcano_uids_to_remove_from_cache}")
    ...
    logging.info(f"Unified metrics collection cycle finished. Processed {len(current_job_uids_processed)} unique job UIDs. Cycle duration: {duration_cycle:.2f}s")
